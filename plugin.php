...
defined( 'STACKABLE_VERSION' ) || define( 'STACKABLE_VERSION', '3.10.2' );
...
if ( ! function_exists( 'stackable_php_requirement_activation_check' ) ) {
	...
	function stackable_php_requirement_activation_check() {
		if ( version_compare( PHP_VERSION, '7.0.0', '<' ) ) {
			...
			wp_die(
				sprintf(
					__( '%s"Stackable" can not be activated. %s It requires PHP version 7.0.0 or higher, but PHP version %s is used on the site. Please upgrade your PHP version first ✌️ %s Back %s', STACKABLE_I18N ),
					'<strong>',
					'</strong><br><br>',
					PHP_VERSION,
	...
	if ( ! function_exists( 'stackable_php_requirement_notice' ) ) {
		function stackable_php_requirement_notice() {
	        printf(
	            ...
	            sprintf( __( '"Stackable" requires PHP version 7.0.0 or higher, but PHP version %s is used on the site.', STACKABLE_I18N ), PHP_VERSION )
	        );
		}
	}
