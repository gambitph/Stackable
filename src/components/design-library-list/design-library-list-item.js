...
const DesignLibraryListItem = forwardRef( ( props, ref ) => {
	...
	const adjustPatternSpacing = attributes => {
	...
	const renderPreview = ( blockContent = content ) => {
		...
		if ( enableBackground && ! selectedNum ) {
			blocks = addBackgroundScheme( blocks, enableBackground, backgroundScheme, addHasBackground )
		}

		if ( selectedTab === 'patterns' ) {
			adjustPatternSpacing( blocks[ 0 ].attributes )
		}
	...
	const replacePlaceholders = ( designContent, designCategory ) => {
	...
	useEffect( () => {
		...
		const initialize = async () => {
			...
			if ( selectedTab === 'patterns' ) {
				_content = replacePlaceholders( _content, category )
				...
				for ( let i = 0; i < _content.length; i++ ) {
					...
					adjustPatternSpacing( _block.attributes )
					_blocks.push( _block )
				}
			}
		}
		...
		initialize().then( () => {
			...
			const { blocks, blocksForSubstitution } = parseDisabledBlocks( _blocks )
