...
register( createReduxStore( 'stackable/global-buttons-and-icons', {
...
domReady( () => {
	fetchSettings().then( response => {
		const {
			stackable_global_buttons_and_icons: _blockLayouts,
		} = response
		const blockLayouts = _blockLayouts || []

		dispatch( 'stackable/global-buttons-and-icons' ).updateBlockLayouts( blockLayouts )
	} )
} )
