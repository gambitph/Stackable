...
/**
 * Checks the attributes whether the value is empty.
 *
 * @param {string} _attrName The attribute name to check
 * @param {Object} attributes The block's attributes object
 * @param {Object} options additional options when checking attributes
 * @return {boolean} True if the attribute doesn't have a value, false if it has a value
 */
export const attributeHasValue = ( _attrName, attributes, options = {} ) => {
	const {
		attrNameTemplate = '',
		hasReponsive = false,
		hasHover = false,
	...
	const attrName = ! attrNameTemplate || attrNameTemplate === '%s' ? _attrName : getAttrName( attrNameTemplate, _attrName )

	if ( ! hasReponsive && ! hasHover ) {
		const value = attributes[ attrName ]
		return value !== '' && typeof value !== 'undefined'
	}

	if ( hasReponsive && ! hasHover ) {
		return ATTR_NAME_MATRIX[ 0 ].some( x => {
			const value = attributes[ `${ attrName }${ x }` ]
			return value !== '' && typeof value !== 'undefined'
		} )
	}

	if ( ! hasReponsive && hasHover ) {
		return ATTR_NAME_MATRIX[ 2 ].some( x => {
			const value = attributes[ `${ attrName }${ x }` ]
			return value !== '' && typeof value !== 'undefined'
		} )
	}
	...
	return ATTR_NAME_MATRIX[ 0 ].some( x => {
		return ATTR_NAME_MATRIX[ 2 ].some( y => {
